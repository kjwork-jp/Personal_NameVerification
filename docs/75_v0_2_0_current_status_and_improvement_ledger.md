...
| restore/import | JSON importはPASS。現在利用中DBへのGUI restoreは `RESTORE-LOCK-001` で事前ブロック済み。invalid input確認は残件 |
...
| P1 | RESTORE-LOCK-001 | 現在利用中DBへのGUI restoreブロック | 完了 | PR #143。confirm前・restore service呼出前に停止。品質ゲートPASS |
| P1 | INVALID-IO-001 | invalid restore/import input再確認 | 未完 | invalid input時に退避DBを作らず、Operations Logへ失敗が残ることを確認 |
...
| admin | 一部OK | user管理/restore/import表示OK。最後のadmin保護OK。現在DBへのrestoreはGUIで事前ブロック済み。invalid input確認は未完 |
...
| 順序 | 作業 | 完了条件 |
|---:|---|---|
| 1 | PR #143 merge判断 | RESTORE-LOCK-001差分と品質ゲートPASSを確認してmergeする |
| 2 | invalid restore/import input再確認 | invalid input時に退避DBを作らず、Operations Logへ失敗が残る |
| 3 | SQL dump保護方針整理 | full DB dumpの扱い、sanitized export要否、警告文言を決定 |
