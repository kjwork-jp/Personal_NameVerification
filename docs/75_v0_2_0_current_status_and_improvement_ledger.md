...
| restore/import | JSON importはPASS。現在利用中DBへのGUI restoreは `RESTORE-LOCK-001` で事前ブロック済み。invalid restore/import input証跡テストは追加済み。ローカル品質ゲート再確認待ち |
...
| 品質ゲート | 2026-05-21時点で `pytest -q` / `ruff check .` / `black --check .` / `mypy app` 全PASS確認済み。INVALID-IO-001追加テスト後の再実行待ち |
...
| 19 | `INVALID-IO-001` としてinvalid restore/import inputの退避DB未作成・Operations Log error記録の証跡テストを追加 | 確認中 |
...
| 認証 | login異常系UAT | 完了 | 誤password、未登録operator_id、disabled user、空operator_idを確認 |
| 管理者保護 | 最後の有効admin降格/無効化保護 | 完了 | GUIで拒否を確認。削除操作は現UIで対象外 |
| 監査 | login/user管理audit | 一部完了 | login_failure、disable、enable、role_change、password非記録を確認。DB変更ログ空ファイルは仕様確認対象 |
...
| QA | invalid destructive I/O証跡テスト | 確認中 | invalid restore/import input時のbefore backup未作成とOperations Log error記録を追加 |
...
| 優先 | ID | 内容 | 状態 | 次アクション |
|---:|---|---|---|---|
| P0 | GATE-001 | 最新main品質ゲート再確認 | 完了 | `pytest -q` / `ruff check .` / `black --check .` / `mypy app` 全PASS |
...
| P1 | AUTH-002 | login異常系UAT | 完了 | 誤password、未登録operator_id、disabled user、空operator_id確認済み |
| P1 | ADMIN-001 | 最後の有効admin保護UAT | 完了 | 降格/無効化拒否を確認。削除は現UI対象外 |
| P1 | DATAIO-002 | Export/Backup/Operations Log実行UAT | 完了 | admin/editorで実出力・バックアップ・ログ確認済み |
| P1 | AUDIT-002 | login_failure / role_change / disable / enable の監査ログUAT | 完了 | user_audit_logs画面/DB/スクリーンショットで確認済み |
| P1 | RESTORE-LOCK-001 | 現在利用中DBへのGUI restoreブロック | 完了 | confirm前・restore service呼出前に停止。品質ゲートPASS |
| P1 | INVALID-IO-001 | invalid restore/import input再確認 | 確認中 | 証跡テスト追加済み。品質ゲート再実行待ち |
| P1 | EXPORT-SEC-001 | SQL dumpの機微情報扱い整理 | 未完 | SQL dumpにusers/password hash/salt等が含まれるため、保護警告またはsanitized export方針を決定 |
...
| 区分 | 状態 | メモ |
|---|---|---|
| 品質ゲート | 要再確認 | INVALID-IO-001証跡テスト追加後の `pytest -q` / `ruff check .` / `black --check .` / `mypy app` 再実行待ち |
...
| admin | 一部OK | user管理/restore/import表示OK。最後のadmin保護OK。現在DBへのrestoreはGUIで事前ブロック済み。invalid input証跡テスト追加済み |
...
| 順序 | 作業 | 完了条件 |
|---:|---|---|
| 1 | INVALID-IO-001品質ゲート再確認 | `pytest -q` / `ruff check .` / `black --check .` / `mypy app` が全PASS |
| 2 | SQL dump保護方針整理 | full DB dumpの扱い、sanitized export要否、警告文言を決定 |
| 3 | 最終Go/No-Go判定 | release blockerなしを確認 |
