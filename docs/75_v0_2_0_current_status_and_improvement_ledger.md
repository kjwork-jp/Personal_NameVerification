...
| QA | invalid destructive I/O証跡テスト | 完了 | invalid restore/import input時のbefore backup未作成とOperations Log error記録を追加。品質ゲートPASS |
...
| P1 | INVALID-IO-001 | invalid restore/import input再確認 | 完了 | 証跡テスト追加済み。品質ゲートPASS |
| P1 | EXPORT-SEC-001 | SQL dumpの機微情報扱い整理 | 未完 | SQL dumpにusers/password hash/salt等が含まれるため、保護警告またはsanitized export方針を決定 |
| P1 | RELEASE-001 | release evidence最終更新 | 未完 | EXPORT-SEC-001方針反映後にdocs/78を最終固定 |
...
| 順序 | 作業 | 完了条件 |
|---:|---|---|
| 1 | SQL dump保護方針整理 | full DB dumpの扱い、sanitized export要否、警告文言を決定 |
| 2 | 必要に応じてSQL dump保護警告またはsanitized exportを実装 | 品質ゲートPASS |
| 3 | release evidence最終固定 | docs/75 / docs/78 / docs/97 を最終同期 |
