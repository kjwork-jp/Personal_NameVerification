...
`docs/72` はUAT実行記録、`docs/73` はUI再設計計画、`docs/74` はRBAC定義、`docs/78` はrelease evidence、`docs/89` はviewer UAT補足、`docs/90` はeditor UAT follow-up、`docs/97` は未解決課題を扱う。本文書はそれらを横断し、ここまでの流れ、決定事項、実装済み事項、残件を整理する現況台帳である。
...
| アカウント切替 | DB close問題は修正済み。viewer切替後のログイン問題も解消済み |
| viewer | 主要更新系・destructive系・Restore/Import系実行ボタンは非表示。参照専用としてUAT証跡化済み |
| editor | 通常登録/更新、関連付け登録、関連付け解除、export/backupを許可。destructive/import/restore/user管理/user audit log操作は禁止 |
| admin | destructive/import/restore/user管理/user audit logを含めた管理者権限。最後の有効admin降格・無効化保護はP1 UAT PASS |
| UI構造 | 監査ログ統合、タイトル/サブタイトル統合基盤、focused title/subtitle wrapper、データ入出力権限別表示、関連付け表示改善を反映済み |
| editor UAT follow-up | サブタイトル追加/編集、親タイトル検索、関連付け解除、説明ラベル折返し修正はmain反映済み。editor再UAT待ち |
...
| 品質ゲート | editor UAT follow-up後、`6d9550f75dd1ecec32f9b7d9a2a084d8e76f6e48` / run `26549117150` で pytest / ruff / black / mypy 全PASS |
| release readiness | コード品質ゲートはPASS。editor再UATと最終docs同期が未完了のため、release判定はBLOCKED |
...
| 20 | `EXPORT-SEC-001` としてSQL dumpの保護警告をUIへ追加。full DB dumpは維持し、sanitized exportは将来課題化 | 完了 |
| 21 | viewer UAT証跡の誤解釈を修正し、実行ボタン非表示による制御として `docs/89` に記録 | 完了 |
| 22 | editor UATでサブタイトル/関連解除/UI崩れを検出し、修正・テスト・品質ゲートPASSまで完了 | 再UAT待ち |
| 23 | Actions結果をCommit Status `chatgpt/quality-gates` として発行する仕組みを追加 | 完了 |
...
| UI | viewer実行ボタン非表示 | 完了 | viewerでは復元・完全削除・Restore・Import等を非表示として証跡化 |
| UI | 関連付け表示改善 | 完了 | 名前/タイトル/サブタイトル/公開ID表記を改善 |
| UI | editor関連付け解除 | 完了 | editor/adminで通常リンク解除を許可。viewerは不可 |
| UI | サブタイトル親タイトル検索 | 完了 | `QComboBox` editable + completerで部分一致検索可能化 |
| UI | タイトル/サブタイトル説明ラベル折返し | 完了 | focused wrapper側で説明文の折返し・短縮を実施 |
...
| RBAC | viewer/editor/admin制御 | 完了 | viewerは参照専用、editorは通常更新/export/backup/関連解除、adminは管理操作可 |
| QA | release remaining UAT自動テスト | 完了 | ID表示、RBAC表示、統合タブ確認を自動化 |
| QA | editor UAT follow-up自動テスト | 完了 | サブタイトル、関連解除、UI説明ラベルを補強 |
...
| QA | Commit Status連携 | 完了 | `chatgpt/quality-gates` でActions結果を外部確認可能化 |
...
| 優先 | ID | 内容 | 状態 | 次アクション |
|---:|---|---|---|---|
| P1 | BUG-SUBTITLE-001-UAT | サブタイトル追加・編集の再UAT | 未完 | editorで追加・編集を実行し、結果を証跡化 |
| P1 | UX-SUBTITLE-002-UAT | サブタイトル親タイトル検索の再UAT | 未完 | タイトル名の一部で検索できることを確認 |
| P1 | BUG-LINK-001-UAT | 関連付け解除の再UAT | 未完 | editorで既存関連付けを解除できることを確認 |
| P1 | BUG-EDITOR-001-UAT | UI崩れ軽減の再UAT | 未完 | タイトル編集・説明コメント周辺の見え方を確認 |
| P1 | DOC-SYNC-001 | UAT/docs最終同期 | 進行中 | `docs/72` / `docs/75` / `docs/88` / `docs/89` / `docs/90` / `docs/97` を同期 |
| P1 | RELEASE-001 | release Go/No-Go判定 | 未完 | editor再UATとdocs同期後に判定 |
...
| editor | 可 | 可 | 可 | 可 | 不可 | 可 | 可 | 不可 | 不可 | 不可/内容非表示 |
...
| 区分 | 状態 | メモ |
|---|---|---|
| 品質ゲート | OK | `6d9550f75dd1ecec32f9b7d9a2a084d8e76f6e48` / run `26549117150` で pytest / ruff / black / mypy 全PASS |
| GUI起動 | OK | 通常起動・通常終了でPowerShell復帰確認済み |
| アカウント切替 | OK | DB close問題は解消済み。白画面・小窓・プロセス残留なし |
...
| viewer | OK | 復元・完全削除・Restore・Import等の実行ボタン非表示を証跡化済み |
| editor | 再UAT待ち | 修正は反映済み。サブタイトル追加/編集、親タイトル検索、関連解除、UI崩れ軽減の再確認が必要 |
...
| 順序 | 作業 | 完了条件 |
|---:|---|---|
| 1 | editor再UAT | サブタイトル追加/編集、親タイトル検索、関連解除、UI崩れ軽減がPASS |
| 2 | UAT/docs最終同期 | `docs/72` / `docs/75` / `docs/88` / `docs/89` / `docs/90` / `docs/97` の整合完了 |
| 3 | release evidence最終固定 | docs/78を最終固定し、配布対象zip方針を確定 |
| 4 | 最終Go/No-Go判定 | release blockerなしを確認 |
...
| `docs/75_v0_2_0_current_status_and_improvement_ledger.md` | 現況/改善台帳 |
...
| `docs/89_account_switch_viewer_uat_20260527.md` | viewer UAT / account switch補足証跡 |
| `docs/90_editor_uat_followup_20260527.md` | editor UAT follow-up証跡 |
