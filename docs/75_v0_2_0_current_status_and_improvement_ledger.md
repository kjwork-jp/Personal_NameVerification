...
| restore/import | JSON importはPASS。ただしGUIから現在利用中DBをrestoreするとWinError 5 access deniedで失敗するため `RESTORE-001` はblocker扱い |
...
| P1 | RESTORE-001 | Restore/Import破壊操作UAT | 一部NG | JSON importは成功。GUI restoreはWinError 5で失敗。invalid input確認は追加要 |
| P1 | RESTORE-FIX-001 | 現在利用中DB restore方式の修正 | 未完・blocker | 接続解放後restore、またはoffline restore導線へ設計変更 |
...
| admin | 一部NG | user管理/restore/import表示OK。最後のadmin保護OK。restore実行はWinError 5でNG |
...
| 順序 | 作業 | 完了条件 |
|---:|---|---|
| 1 | RESTORE-FIX-001方針決定 | in-app restoreの現在DB置換を接続解放して許可するか、offline restoreへ誘導するか決定 |
| 2 | restore修正PR | WinError 5が再発せず、restore成功または明確にブロックされる |
| 3 | invalid restore/import input再確認 | invalid input時に退避DBを作らず、Operations Logへ失敗が残る |
| 4 | SQL dump保護方針整理 | full DB dumpの扱い、sanitized export要否、警告文言を決定 |
| 5 | P1残件台帳更新 | `docs/78` と `docs/97` を最新化 |
