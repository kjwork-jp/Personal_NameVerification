...
| 品質ゲート | 2026-05-21時点で `pytest -q` / `ruff check .` / `black --check .` / `mypy app` 全PASS確認済み。EXPORT-SEC-001追加後の再実行待ち |
...
| 20 | `EXPORT-SEC-001` としてSQL dumpの保護警告をUIへ追加。full DB dumpは維持し、sanitized exportは将来課題化 | 確認中 |
...
| UI | SQL dump保護警告 | 確認中 | SQL dumpボタン/パス入力tooltipと実行時メッセージに保護警告を表示 |
...
| QA | SQL dump保護警告テスト | 確認中 | tooltip / 実行時警告 / idempotentを追加 |
...
| 優先 | ID | 内容 | 状態 | 次アクション |
|---:|---|---|---|---|
| P1 | EXPORT-SEC-001 | SQL dumpの機微情報扱い整理 | 確認中 | UI警告とテスト追加済み。品質ゲート再実行待ち |
| P1 | RELEASE-001 | release evidence最終更新 | 未完 | EXPORT-SEC-001品質ゲートPASS後にdocs/78を最終固定 |
...
| 区分 | 状態 | メモ |
|---|---|---|
| 品質ゲート | 要再確認 | EXPORT-SEC-001追加後の `pytest -q` / `ruff check .` / `black --check .` / `mypy app` 再実行待ち |
...
| 順序 | 作業 | 完了条件 |
|---:|---|---|
| 1 | EXPORT-SEC-001品質ゲート再確認 | `pytest -q` / `ruff check .` / `black --check .` / `mypy app` が全PASS |
| 2 | release evidence最終固定 | docs/75 / docs/78 / docs/97 を最終同期 |
