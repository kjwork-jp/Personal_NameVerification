...
まずは名前管理画面の詳細欄に `公開ID` を併記する。
内部IDは既存処理互換のため保持し、テーブル上では引き続き非表示にする。

この段階では、更新・削除・復元などの操作対象は従来どおり内部IDで扱う。
UI上のすべてのID表示を一括置換するのではなく、画面ごとに段階的に進める。

## 注意

今回の段階では、Title / Subtitle / Link / Audit Log のUI表示置換はまだ完了していない。

## 次段階

1. export/importで `public_id` を保持する。
2. `public_id` で参照・検索できるqueryを追加する。
3. タイトル・サブタイトル・関連付け・操作履歴の表示を段階的に `public_id` へ寄せる。
