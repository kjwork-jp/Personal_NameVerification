...
| PR | タイトル案 | 内容 | main after PR #141 状態 |
|---|---|---|---|
| PR-020-001 | `feat: add auth schema migrations` | schema_migrations/users/app_settings/user_audit_logs | 実装済み（PR #126） |
| PR-020-002 | `feat: add password hashing service` | PBKDF2 hash/verify | 実装済み（PR #128/#129） |
| PR-020-003 | `feat: add user repository and service` | create/disable/role_change/admin guard | 実装済み（PR #130） |
| PR-020-004 | `feat: add first-run admin setup` | InitialAdminSetupDialog | 実装済み（PR #131） |
| PR-020-005 | `feat: require password login` | LoginDialog password化、role combo削除 | 実装済み（PR #132） |
| PR-020-006 | `feat: add user management tab` | admin専用ユーザー管理 | 実装済み（PR #133） |
| PR-020-007 | `feat: audit authentication and user management` | user_audit_logs | 一部実装済み（PR #134、service側audit含む） |
| PR-020-008 | `feat: add database path protection diagnostics` | path/ACL/平文DB警告 | 未実装/一部README注意喚起のみ |
| PR-020-009 | `test: extend portable smoke for auth setup login` | portable smoke更新 | 一部実装済み（PR #139〜#141でauth tables確認） |
| PR-020-010 | `docs: finalize v0.2.0 release evidence` | release evidence更新 | 未実装 |
...
## 12. 次工程（main after PR #141）

1. v0.2.0認証・ユーザー管理の統合UATを実施する。
2. 初回admin setup、password login、role別操作、user management、user audit log、既存DB migrationをまとめて確認する。
3. DB/backup/export/log保護警告・診断の不足分を実装する。
4. v0.2.0-rc1 packaging / portable smoke / manifest / checksum / release evidence 固定へ進む。
