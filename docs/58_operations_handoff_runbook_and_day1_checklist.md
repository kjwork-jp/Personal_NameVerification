...
- editor:
  ...
  - 運用操作 は export/backup create のみ実行可能。
- admin:
...
### 3.1 業務導線に沿った確認順
以下を **検索 → CRUD → ゴミ箱 → Audit → 運用操作** の順で確認する。
...
4. message 検索（部分一致 or regex）を実施。
   - regex 利用時は `大文字小文字を無視 / 複数行 / ドット改行一致` を必要に応じて切替。
5. 表示順（最新順/古い順）を切替。
6. 前へ/次へ と表示件数（50/100/200/500）で表示内容を確認。
