...
This checklist intentionally separates the items that are expected in the current implementation from the items that are target-state checks for future P0/P1 improvements. This prevents a UAT run from failing current builds for behavior that has not been implemented yet.
...
| Scope | Meaning |
|---|---|
| CURRENT | Expected from the current main branch after the role-navigation work |
| TARGET | Expected only after the related improvement task is implemented |
| REFERENCE | Evidence-gathering only; do not block release by itself |

## 4. Evidence naming policy
...
| ID | Scope | Status | Area | Primary role | Evidence / Notes |
|---|---|---|---|---|---|
| IMG-UAT-001 | CURRENT | NOT RUN | Viewer visible features | Viewer |  |
| IMG-UAT-002 | CURRENT | NOT RUN | Editor visible features | Editor |  |
| IMG-UAT-003 | CURRENT | NOT RUN | Admin visible features | Admin |  |
| IMG-UAT-004 | TARGET | NOT RUN | Duplicate name/title/subtitle prevention | Editor or Admin |  |
| IMG-UAT-005 | TARGET | NOT RUN | Title edit screen layout | Editor or Admin |  |
| IMG-UAT-006 | TARGET | NOT RUN | Subtitle edit from subtitle-focused workflow | Editor or Admin |  |
| IMG-UAT-007 | TARGET | NOT RUN | Minimal list columns | Viewer, Editor, Admin as needed |  |
| IMG-UAT-008 | TARGET | NOT RUN | Full information in detail pane / selection card | Viewer, Editor, Admin as needed |  |
| IMG-UAT-009 | TARGET | NOT RUN | Audit log diff readability | Admin |  |
| IMG-UAT-010 | TARGET | NOT RUN | Datetime display format | Viewer, Editor, Admin as needed |  |

## 7. Current-build checks
...
### IMG-UAT-002: Editor does not show admin-only feature areas
...
| Notes | Older requirements may still mention editor export/backup access. The 2026-06-04 screenshot review policy supersedes that UI expectation for the current role-navigation work. A narrower editor export-only flow can be designed as a separate future task if needed. |
...
## 8. Target-state checks for future improvement work
...
|---|---|
| Scope | TARGET |
| Applies after | Unique-name/title/subtitle policy and implementation are merged. |
...
| Current-build handling | Until the unique constraint and service validation work is merged, this check is evidence-gathering only. Do not fail the current build solely because title or subtitle duplicates can still be created. |
| NG examples | Duplicate name/title/subtitle is registered successfully after the unique-constraint task is complete. Duplicate detection only happens after the list already contains the new duplicate. The message is vague, missing, or points to the wrong entity. Duplicate subtitle is blocked globally when it should only be judged within the correct parent-title context, or allowed within the same parent when it should be blocked. |
...
|---|---|
| Scope | TARGET |
| Applies after | Title edit layout fix is merged. |
...
|---|---|
| Scope | TARGET |
| Applies after | Subtitle-focused edit workflow is merged. |
...
|---|---|
| Scope | TARGET |
| Applies after | List-column reduction work is merged. |
...
|---|---|
| Scope | TARGET |
| Applies after | Detail-pane or selection-card expansion work is merged. |
...
|---|---|
| Scope | TARGET |
| Applies after | Audit-log diff readability work is merged. |
...
|---|---|
| Scope | TARGET |
| Applies after | Datetime display helper is wired into the target UI screens. |
...
| Current-build handling | Until the datetime helper is applied to the relevant screens, ISO timestamps may still appear. Treat that as a known gap rather than a current-build release blocker. |
| NG examples | After datetime display integration is complete, datetime appears as `2026-06-04 09:30:05`, `2026-06-04T09:30:05`, `2026/06/04 09:30`, `06/04/2026 09:30:05`, raw epoch values, or a mixture of multiple formats on the same screen. |
...
## 10. UAT exit decision

### Current-build exit

| Item | Status | Notes |
|---|---|---|
| Current checks complete | NOT RUN | IMG-UAT-001 through IMG-UAT-003 are required for current role-navigation UAT. |
| Role visibility screenshots captured | NOT RUN | Viewer, Editor, and Admin evidence are required. |
| No current-build blocker remains | NOT RUN | TARGET checks may remain open if the corresponding implementation is not merged yet. |

### Target-state exit

| Item | Status | Notes |
|---|---|---|
| Duplicate prevention confirmed | NOT RUN | Required after unique constraint/service validation/UI pre-check work is merged. |
| Layout and workflow checks passed | NOT RUN | Required after title layout and subtitle-focused workflow work is merged. |
| List/detail information split confirmed | NOT RUN | Required after list-column reduction/detail-pane work is merged. |
| Audit and datetime display confirmed | NOT RUN | Required after audit diff and datetime display integration work is merged. |
| No target-state blocker remains | NOT RUN | Use after all related P0/P1 improvements are implemented. |
