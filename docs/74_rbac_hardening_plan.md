...
また、editor UAT中にログイン中ユーザーと権限が分かりにくい課題が確認されたため、MainWindow title bar と status bar に現在の `operator_id` / `role` を常時表示する方針を追加した。
...
- 画面上は常に `ログイン中: <operator_id> / 権限: <role>` を表示し、操作権限の誤認を防ぐ。
...
| 視認性 | title bar / status bar でログイン中operator_idとroleを常時表示する |
...
- `MainWindow` では title bar / status bar に `ログイン中: <operator_id> / 権限: <role>` を常時表示する。
...
- `NameManagementTab` はviewerで名前/備考/操作者ID/作成/更新/削除を無効化済み。editorでは通常作成/更新が有効表示、削除は無効表示を確認済み。
- `TitleSubtitleManagementTab` はviewerでタイトル名/備考/関連付ける名前/管理番号/サブタイトル名/表示順/備考/作成/更新/削除を無効化済み。editorでは作成/更新系が有効表示、削除は無効表示を確認済み。
- `LinkManagementTab` はviewerで登録/解除系入力欄とボタンを無効化済み。editorは登録のみ有効表示、解除不可。
- `OperationsTab` はviewerでExport/Backup/Restore/Importの入力欄・参照・履歴削除・実行ボタン・ログエクスポートを無効化済み。editorはExport/Backupが有効、Restore/Importが無効。Operationsログ参照は許可。
...
|---:|---|---|---|---|---|
| P0 | MainWindow共通 | operator_id/role常時表示 | operator_id/role常時表示 | operator_id/role常時表示 | UI反映済み |
| P0 | ユーザー管理 | 操作不可 | 操作不可 | 作成/一覧/操作可。ただし最後のadmin保護あり | UI反映済み。admin詳細UAT未完 |
| P0 | ユーザー監査ログ | 操作不可/内容非表示 | 操作不可/内容非表示 | フィルタ・一覧・詳細確認可 | UI反映済み。admin詳細UAT未完 |
| P0 | データ入出力 | Operationsログ参照のみ。export/backup/import/restore不可 | export/backup可、import/restore不可 | 全操作可 | UI反映済み。実行UAT未完 |
| P0 | 削除データ | 復元/完全削除不可 | 復元/完全削除不可 | 復元/完全削除可 | 既存UIで概ね反映済み。admin詳細UAT未完 |
| P1 | 名前を管理 | 登録/更新/削除不可。編集入力欄も不可 | 登録/更新可、削除不可 | 登録/更新/削除可 | UI反映済み。editor実行UAT未完 |
| P1 | タイトルを管理 | 登録/更新/削除不可。編集入力欄も不可 | 登録/更新可、削除不可 | 登録/更新/削除可 | UI反映済み。editor実行UAT未完 |
| P1 | サブタイトルを管理 | 登録/更新/削除不可。編集入力欄も不可 | 登録/更新可、削除不可 | 登録/更新/削除可 | UI反映済み。editor実行UAT未完 |
| P1 | 関連付け | 登録/解除不可。入力欄も不可 | 登録可、解除不可 | 登録/解除可 | UI反映済み。editor実行UAT未完 |
...
| 2026-05-18 | `61137aa` | RBAC UI guardのruff指摘を修正 |
| 2026-05-18 | `4f4c1d9` | MainWindowにログイン中ユーザー/権限の常時表示を追加 |
...
- title bar / status bar から現在のoperator_id/roleを常時判別できる。
...
| P0 | editor role UAT: 通常作成/更新/関連付け登録/export/backupが可能で、削除/import/restore/ユーザー管理が不可 | UI表示一部OK、実行未実施 |
