...
Write-Host "[1/9] Check portable release inputs"
...
Write-Host "[2/9] Prepare isolated portable smoke folder"
...
Write-Host "[3/9] Validate portable layout"
...
Write-Host "[4/9] Stop stale EXE processes"
Stop-NameVerificationProcesses

Write-Host "[5/9] Launch portable EXE from package root"
...
'@
    ...
    python $TableCheckScriptPath $PortableDbPath "users" "user_audit_logs" "app_settings" "schema_migrations"
    if ($LASTEXITCODE -ne 0) { throw "Portable smoke database table check failed" }

    Write-Host "[8/9] Stop portable EXE"
