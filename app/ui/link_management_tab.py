...
class LinkWriteService(Protocol):
    ...
    def unlink_name_from_subtitle(self, link_id: int, operator_id: str, role: UserRole = "admin") -> None: ...
...
class LinkReadService(Protocol):
    ...
    def list_related_rows(self, name_id: int, role: UserRole = "admin", *, include_deleted: bool = False) -> list[RelatedRow]: ...
...
class LinkManagementTab(QWidget):
    ...
    def __init__(self, core_service: LinkWriteService, query_service: LinkReadService, role_context: RoleContext | None = None) -> None:
        super().__init__()
        ...
        self.custom_relation_type_input.setToolTip("other 選択時に custom relation_type を入力してください")
        ...
        self.links_table.setHorizontalHeaderLabels(["リンクID", "タイトル", "コード", "relation_type"])
        ...
        left = QWidget(); left_l = QVBoxLayout(left); left_l.addWidget(QLabel("名前")); left_l.addWidget(self.names_table)
        middle = QWidget(); mid_l = QVBoxLayout(middle); mid_l.addWidget(QLabel("タイトル")); mid_l.addWidget(self.titles_table); mid_l.addWidget(QLabel("サブタイトル")); mid_l.addWidget(self.subtitles_table)
        right = QWidget(); right_l = QVBoxLayout(right); right_l.addWidget(QLabel("既存リンク")); right_l.addWidget(self.links_table)
        splitter = QSplitter(); splitter.addWidget(left); splitter.addWidget(middle); splitter.addWidget(right)
        root = QVBoxLayout(self)
        root.addLayout(form); root.addLayout(actions); root.addWidget(self.message_label); root.addWidget(splitter)
        self._apply_role_guards(); self._refresh_all()
    ...
    def _apply_role_guards(self) -> None:
        ...
        self.link_button.setToolTip("このロールでは実行できません" if not can_link(role) else "operator_id・relation_type・Name/Subtitle選択が必要です")
        self.unlink_button.setToolTip("このロールでは実行できません" if not can_unlink(role) else "解除対象リンクを選択してください")

    def _refresh_all(self) -> None:
        self._names = self._query_service.search_names(include_deleted=False, role=self._role_context.role)
        self._titles = self._query_service.list_titles(include_deleted=False, role=self._role_context.role)
        self.names_table.setRowCount(len(self._names))
        for i, row in enumerate(self._names):
        ...
        self._selected_name = None; self._selected_title = None; self._selected_subtitle = None; self._selected_link = None
        self.subtitles_table.setRowCount(0); self.links_table.setRowCount(0)
        if self._names:
            self.names_table.selectRow(0); self._on_name_selected()
        if self._titles:
            self.titles_table.selectRow(0); self._on_title_selected()

    def _on_name_selected(self) -> None:
        idx = self.names_table.currentRow()
        self._selected_name = _Selection(self._names[idx].id) if 0 <= idx < len(self._names) else None
        self._refresh_links()

    def _on_title_selected(self) -> None:
        idx = self.titles_table.currentRow()
        if 0 <= idx < len(self._titles):
            self._selected_title = _Selection(self._titles[idx].id)
            self._refresh_subtitles(self._titles[idx].id)
        else:
            self._selected_title = None; self.subtitles_table.setRowCount(0)

    def _on_subtitle_selected(self) -> None:
        idx = self.subtitles_table.currentRow()
        self._selected_subtitle = _Selection(self._subtitles[idx].id) if 0 <= idx < len(self._subtitles) else None

    def _on_link_selected(self) -> None:
        idx = self.links_table.currentRow()
        self._selected_link = _Selection(self._links[idx].link_id) if 0 <= idx < len(self._links) else None
    ...
    def _on_relation_type_changed(self) -> None:
        ...
        self.custom_relation_type_input.setToolTip("custom relation_type を入力してください" if is_custom else "other 選択時のみ入力できます")
        if not is_custom:
            self.custom_relation_type_input.clear()

    def _refresh_subtitles(self, title_id: int) -> None:
        self._subtitles = self._query_service.list_subtitles(title_id, include_deleted=False, role=self._role_context.role)
        ...
        if self._subtitles:
            self.subtitles_table.selectRow(0); self._on_subtitle_selected()

    def _refresh_links(self) -> None:
        if self._selected_name is None:
            self.links_table.setRowCount(0); return
        self._links = self._query_service.list_related_rows(self._selected_name.id, include_deleted=False, role=self._role_context.role)
        ...
        if self._links:
            self.links_table.selectRow(0); self._on_link_selected()

    def _create_link(self) -> None:
        if not can_link(self._role_context.role):
            self._set_message("このロールではリンク作成できません", is_error=True); return
        operator_id = self._require_operator_id(); relation_type = self._selected_relation_type()
        if operator_id is None or relation_type is None:
            return
        if self._selected_name is None or self._selected_subtitle is None:
            self._set_message("Name と Subtitle を選択してください", is_error=True); return
        self._core_service.link_name_to_subtitle(self._selected_name.id, self._selected_subtitle.id, relation_type=relation_type, operator_id=operator_id, role=self._role_context.role)
        self._set_message("リンク作成しました"); self._refresh_links()

    def _unlink_link(self) -> None:
        if not can_unlink(self._role_context.role):
            self._set_message("このロールではリンク解除できません", is_error=True); return
        ...
        if self._selected_link is None:
            self._set_message("解除するリンクを選択してください", is_error=True); return
        if not confirm_destructive_action(self, "リンク解除の確認", f"リンクID={self._selected_link.id} を解除します。よろしいですか？"):
            self._set_message("リンク解除をキャンセルしました"); return
        self._core_service.unlink_name_from_subtitle(self._selected_link.id, operator_id=operator_id, role=self._role_context.role)
        self._set_message("リンク解除しました"); self._refresh_links()

    def _selected_relation_type(self) -> str | None:
        selected_value = str(self.relation_type_combo.currentData())
        if not selected_value:
            self._set_message("relation_type を選択してください", is_error=True); return None
        if selected_value == "other":
            custom_value = self.custom_relation_type_input.text().strip()
            if not custom_value:
                self._set_message("custom relation_type を入力してください", is_error=True); return None
            return custom_value
    ...
    def _require_operator_id(self) -> str | None:
        ...
        if not operator_id:
            self._set_message("操作者ID を入力してください", is_error=True); return None
        return operator_id
