...
class LinkManagementTab(QWidget):
    ...
    def _refresh_all(self) -> None:
        self._names = _call_with_optional_role(
    ...
    def _on_name_selected(self) -> None:
        ...
        if 0 <= idx < len(self._names):
            self._selected_name = _Selection(self._names[idx].id)
        else:
            self._selected_name = None
        self._refresh_links()
    ...
    def _on_title_selected(self) -> None:
        ...
        if 0 <= idx < len(self._titles):
            selected_title = self._titles[idx]
            self._selected_title = _Selection(selected_title.id)
            self._refresh_subtitles(selected_title.id)
        else:
            self._selected_title = None
            self.subtitles_table.setRowCount(0)
    ...
    def _on_subtitle_selected(self) -> None:
        ...
        if 0 <= idx < len(self._subtitles):
            self._selected_subtitle = _Selection(self._subtitles[idx].id)
        else:
            self._selected_subtitle = None
    ...
    def _on_link_selected(self) -> None:
        ...
        if 0 <= idx < len(self._links):
            self._selected_link = _Selection(self._links[idx].link_id)
        else:
            self._selected_link = None
    ...
    def _refresh_links(self) -> None:
        if self._selected_name is None:
            self.links_table.setRowCount(0)
            return
        self._links = _call_with_optional_role(
