...
def _trash() -> None:
    ...
    def _entity_key(self: Any) -> str:
        return {"名前": "name", "Name": "name", "タイトル": "title", "Title": "title", "サブタイトル": "subtitle", "Subtitle": "subtitle", "リンク": "link", "Link": "link"}.get(self.entity_selector.currentText(), "name")
    ...
    def _restore_selected(self: Any) -> None:
        ...
        if operator_id is None or selection is None:
            return
        ...
        getattr(self._core_service, method)(selection.entity_id, operator_id, role=self._role_context.role)
