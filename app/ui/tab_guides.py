...
_GUIDES: dict[str, str] = {
    "検索": "名前・タイトル・サブタイトルを横断検索します。条件を入力して検索し、下段で選択中の関連タイトル/サブタイトルを確認します。",
    "名前を管理": "名前の一覧確認、絞り込み、新規作成、更新、ゴミ箱移動を行います。復元と完全削除は削除データタブで実行します。",
    "タイトルを管理": "名前を選択し、関連するタイトルを登録・更新します。削除済みデータの復元/完全削除は削除データタブで実行します。",
    "サブタイトルを管理": "タイトルを選択し、配下のサブタイトルを登録・更新します。管理番号は未入力なら自動生成されます。",
    "関連付け": "名前とタイトル/サブタイトルの関連付けを作成・確認します。関連解除や削除系操作は権限に応じて制御されます。",
    "削除データ": "削除済みデータを対象別に確認し、adminのみ復元または完全削除を実行できます。対象別サブタブで表示対象を切り替えます。",
    "操作履歴": "変更履歴を確認します。登録・更新・削除・復元などの操作証跡を追跡するための参照画面です。",
    "ユーザー管理": "ユーザーの作成、ロール変更、無効化、パスワード再設定を行います。最後の有効admin保護が適用されます。",
    "ユーザー監査ログ": "ログイン失敗、ユーザー管理操作などの認証・ユーザー系監査ログを確認します。",
    "データ入出力": "Export / Backup / Restore / Import / Operations Log を用途別サブタブから実行します。Restore/Importはadmin専用です。",
    "ヘルプ / 設定": "実行中のDB、ログ、パッケージパスを確認します。配布時やトラブル調査時の参照画面です。",
}
