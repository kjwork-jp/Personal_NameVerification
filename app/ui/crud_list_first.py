...
def apply_crud_list_first(widget: QtWidgets.QWidget, tab_title: str) -> None:
...
def _mark_relationship_tab(widget: QtWidgets.QWidget) -> None:
    widget.setProperty("relationship_workflow_guided", True)


def _insert_list_first_hint(widget: QtWidgets.QWidget, tab_title: str) -> None:
...
def _move_child_table_to_top(
    ...
    *,
    ...
    if not isinstance(parent, QtWidgets.QWidget) or not isinstance(table, QtWidgets.QTableWidget):
        return
...
def _hint_insert_index(layout: QtWidgets.QVBoxLayout) -> int:
    """Insert after the page header where possible."""

    return min(1, layout.count())


def _list_insert_index(layout: QtWidgets.QVBoxLayout) -> int:
