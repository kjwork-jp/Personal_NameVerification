...
def test_title_name_blank_input_is_validation_error() -> None:
    _, service = _service()
...
def test_title_name_duplicate_scan_ignores_legacy_blank_rows() -> None:
    conn, service = _service()
    conn.execute(
        """
...
def test_subtitle_name_blank_input_is_validation_error() -> None:
    _, service = _service()
    title_id = service.create_title(TitleInput(title_name="Main"), operator_id="op-1")
...
def test_subtitle_name_duplicate_scan_ignores_legacy_blank_rows() -> None:
    conn, service = _service()
    title_id = service.create_title(TitleInput(title_name="Main"), operator_id="op-1")
    conn.execute(
        """
